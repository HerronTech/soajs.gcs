...
var maxFileUpload = process.env.SOAJS_GC_MAX_UPLOAD_LIMIT || 2 * 1024 * 1024;
...
dataHandler.prototype.buildService = function (cb) {
	...
	this.service.init(function () {
		...
		self.service.post("/upload", function (req, res) {
			...
			dataMw.saveUploadedFile(req, form, function (error) {
				if (error) {
		...
		for (var oneAPI in self.serviceAPIRoutes) {
			if (Object.hasOwnProperty.call(self.serviceAPIRoutes, oneAPI)) {
				...
				self.service.log.debug('Registering:', self.serviceAPIRoutes[oneAPI].method, oneAPI);
...
dataHandler.prototype.stopService = function(cb){
	this.service.stop(cb);
};
