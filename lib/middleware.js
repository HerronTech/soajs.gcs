...
function checkForMongoCore(config, req, cb) {
	...
	mongoCore.findOne("environment", {"code": req.soajs.inputmaskData['__env'].toUpperCase()}, {"dbs": 1}, function(error, envInfo) {
		...
		if(!envInfo){ return cb(new Error('This Environment has no Database Connection') ); }
		for(var dbName in envInfo.dbs.databases) {
			if(dbName === config.serviceName) {
				connectionInfo = envInfo.dbs.clusters[envInfo.dbs.databases[dbName].cluster];
				connectionInfo.name = config.serviceName;

				if(envInfo.dbs.databases[dbName].tenantSpecific) {
					var tenantCode = req.soajs.session.getUrac().tenant.code;
					connectionInfo.name = tenantCode + '_' + config.serviceName;
				}
			}
		}
		if(!connectionInfo){ return cb(new Error('This Environment has no Database Connection') ); }
...
var dataMw = function(config) {
		...
		'initialize': function(req, res, next) {
			...
			if(self.config.db.multitenant) {
				checkForMongoCore(config, req, function(error, connectionInfo) {
					if(error) { res.jsonp(req.soajs.buildResponse({"code": self.config.mw.list.code, "msg": error.message})); }
	...
	this.get = {
		'initialize': function(req, res, next) {
			...
			if(self.config.db.multitenant) {
				checkForMongoCore(config, req, function(error, connectionInfo) {
					if(error){ return res.jsonp(req.soajs.buildResponse({"code": self.config.mw.get.code, "msg": error.message})); }
					self.mongo = new Mongo(connectionInfo);
					proceed();
				});
			}
	...
	this.delete = {
		'initialize': function(req, res, next) {
			...
			if(self.config.db.multitenant) {
				checkForMongoCore(config, req, function(error, connectionInfo) {
					if(error) { return res.jsonp(req.soajs.buildResponse({"code": self.config.mw.delete.code, "msg": error.message})); }
					self.mongo = new Mongo(connectionInfo);
					proceed();
				});
			}
	...
	this.update = {
		'initialize': function(req, res, next) {
			...
			if(self.config.db.multitenant) {
				...
				checkForMongoCore(config, req, function(error, connectionInfo) {
					if(error) { return res.jsonp(req.soajs.buildResponse({"code": self.config.mw.update.code, "msg": error.message})); }
					self.mongo = new Mongo(connectionInfo);
					proceed();
				});
			}
