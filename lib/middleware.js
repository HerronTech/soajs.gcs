...
var dataMw = function (config, formFields) {
        ...
        'exec': function (req, res, next) {
            self.mongo.findOne(self.context.db.collection, self.context.db.condition || {}, function (error, oneRecord) {
                ...
                if (Object.keys(fileFields).length > 0) {
                    self.getUploadedFileInfo(fileFields, oneRecord._id.toString(), function (error) {
                        if (error) {
                            if (self.config.db.multitenant) {
                                self.mongo.closeDb();
                            }
    ...
    this.delete = {
        ...
        'exec': function (req, res, next) {
            self.removeUploadedFile(req.soajs.inputmaskData.id, function (error) {
                if (error) {
                    if (self.config.db.multitenant) {
                        self.mongo.closeDb();
                    }
    ...
    this.saveUploadedFile = function (req, res, fileInfo, cb) {
        ...
        function proceed() {
            ...
            self.mongo.getMongoSkinDB(function (error, db) {
                ...
                if (fileInfo.fields.action === 'edit') {
                    removeFileBasedOnPosition(gfs, function (error) {
                        if (error) {
                            return cb(error);
                        }
                        doSaveFile(gfs, condition);
                    });
                }
                else {
                    doSaveFile(gfs, condition);
        ...
        function removeFileBasedOnPosition(gfs, cb) {
            var condition = {
                'metadata.nid': fileInfo.fields.nid,
                'metadata.position': fileInfo.fields.position,
                media: fileInfo.fields.media
            };
        ...
        function doSaveFile(gfs, condition) {
            var writestream = gfs.createWriteStream({
                filename: fileInfo.file.name,
                metadata: {
                    position: fileInfo.fields.position,
                    nid: fileInfo.fields.nid,
                    media: fileInfo.fields.media
                }
            ...
            writestream.on('close', function (file) {
                ...
                update['$push']['fields.' + fileInfo.fields.field] = file._id.toString();
                self.mongo.update(self.context.db.collection, condition, update, {upsert: false}, cb);
            });
        }
    };

    this.removeUploadedFile = function (refId, cb) {
        self.mongo.getMongoSkinDB(function (error, db) {
            if (error) {
                return cb(error);
            }
            ...
            self.mongo.find('fs.files', { 'metadata.nid': refId }, {}, {sort: {'metadata.position': 1}}, function (err, files) {
                ...
                if(files.length === 0){ return cb(null, true); }
                var count = 0;
        ...
        function proceed() {
            self.mongo.getMongoSkinDB(function (error, db) {
                ...
                gfs.remove(condition, function (err) {
                    ...
                    doc['$pull']['fields.' + req.soajs.inputmaskData.n] = req.soajs.inputmaskData.id;
    ...
    this.getUploadedFileInfo = function (fileFields, refId, cb) {
        var count = 0;
        ...
        self.mongo.find('fs.files', condition, {}, {sort: {'metadata.position': 1}}, function (err, files) {
            ...
            files.forEach(function (oneFileRecord) {
                for (var oneFileField in fileFields) {
                    if (oneFileField === oneFileRecord.metadata.media) {
        ...
        function proceed() {
            ...
            self.mongo.getMongoSkinDB(function (error, db) {
                ...
                self.mongo.findOne('fs.files', condition, function (error, fileInfo) {
                    ...
                    var gfs = Grid(db, self.mongo.mongoSkin);
                    var gs = new gfs.mongo.GridStore(db, self.mongo.ObjectId(req.soajs.inputmaskData.id), 'r', {
                        root: 'fs',
                        w: 1,
                        fsync: true
                    });
