...
function checkForMongoCore(config, req, cb) {
	...
	mongoCore.findOne("environment", {"code": process.env.SOAJS_ENV.toUpperCase()}, {"dbs": 1}, function (error, envInfo) {
		if (error) {
			return cb(error);
		}
		...
		var dbInfo = config.db.config[process.env.SOAJS_ENV.toUpperCase()];
